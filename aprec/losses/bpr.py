...
class BPRLoss(Loss):
    ...
    def __call__(self, y_true, y_pred):
        ...
        mask = tf.cast(
            (
                get_pairwise_diff_batch(
                    top_true.values, true_ordered_by_pred, self.max_positives, pred_size
                )
                > 0
            ),
            tf.float32,
        )
        values = get_pairwise_diff_batch(
            pred_ordered_by_true, pred, self.max_positives, pred_size
        )
        ...
        if self.softmax_weighted:
            ...
            pred_softmax = tf.transpose(
                masked_softmax(pred_tile, mask_transposed), perm=[0, 2, 1]
            )
