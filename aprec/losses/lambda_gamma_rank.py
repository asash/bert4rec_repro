...
class LambdaGammaRankLoss(Loss):
    ...
    def setup(self):
        ...
        self.dcg_position_discounts = 1.0 / tf.experimental.numpy.log2(
            tf.cast(tf.range(self.pred_truncate_at) + 2, self.dtype)
        )
        self.top_position_discounts = tf.reshape(
            self.dcg_position_discounts[: self.ndcg_at], (self.ndcg_at, 1)
        )
        self.swap_importance = tf.abs(
            self.get_pairwise_diffs_for_vector(self.dcg_position_discounts)
        )
        self.batch_indices = tf.reshape(
            tf.tile(
                tf.expand_dims(tf.range(self.batch_size), 1), [1, self.pred_truncate_at]
            ),
        ...
        self.mask = tf.cast(
            tf.reshape(
                1
                - tf.pad(
                    tf.ones(self.ndcg_at), [[0, self.pred_truncate_at - self.ndcg_at]]
                ),
                (1, self.pred_truncate_at),
            ),
            self.dtype,
        )
    ...
    def __call__(self, y_true_raw, y_pred_raw):
        ...
        def grad(dy):
            ...
            return (
                0 * dy,
                (
                    (1 - self.bce_grad_weight) * lambdarank_lambdas
                    + (bce_lambdas * self.bce_grad_weight)
                )
                * dy,
            )
    ...
    def get_lambdas(self, y_true, y_pred):
        ...
        col_indices_reshaped = tf.reshape(
            sorted_by_score.indices, (self.pred_truncate_at * self.batch_size, 1)
        )
        pred_ordered = sorted_by_score.values
        true_ordered = tf.gather(
            tf.cast(y_true, self.dtype), sorted_by_score.indices, batch_dims=1
        )
        ...
        if self.lambda_normalization:
            ...
            range_is_zero = tf.reshape(
                tf.cast(tf.math.equal(best_score, worst_score), self.dtype),
                (self.batch_size, 1, 1),
            )
            norms = (1 - range_is_zero) * (tf.abs(pairwise_diffs) + 0.01) + (
                range_is_zero
            )
        ...
        top_lambda_sum = tf.pad(
            -tf.reduce_sum(lambda_matrix, axis=1),
            [[0, 0], [0, self.pred_truncate_at - self.ndcg_at]],
        )
        ...
        if self.lambda_normalization:
            ...
            all_lambdas_sum = tf.reshape(
                tf.reduce_sum(tf.abs(lambda_sum_result), axis=(1)), (self.batch_size, 1)
            )
            norm_factor = tf.math.divide_no_nan(
                tf.experimental.numpy.log2(all_lambdas_sum + 1), all_lambdas_sum
            )
        ...
        result_lambdas = tf.scatter_nd(
            indices,
            tf.reshape(lambda_sum, [self.pred_truncate_at * self.batch_size]),
    ...
    def get_inverse_idcg(self, true_ordered):
        ...
        return tf.reshape(
            tf.math.divide_no_nan(tf.cast(1.0, self.dtype), top_k_discounted),
            (self.batch_size, 1, 1),
        )
