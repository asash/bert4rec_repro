...
def evaluate_recommender(
    ...
    for i in tqdm(range(len(all_user_ids)), ascii=True):
        ...
        user_doc = {
            ...
            "predictions": [
                (prediction[0], float(prediction[1])) for prediction in predictions
            ],
        }
        if evaluate_on_samples:
        ...
        for metric in metrics:
            ...
            if evaluate_on_samples:
                sampled_metric_value = metric(
                    sampled_rankings[user_id], test_actions_by_user[user_id]
                )
    ...
    for metric in metric_sum:
        ...
        if evaluate_on_samples:
            sampled_result[metric] = sampled_metric_sum[metric] / len(
                test_actions_by_user
            )
    if evaluate_on_samples:
    ...
    def __call__(self):
        ...
        for recommender_name in self.recommenders:
            try:
                ...
                evaluation_result = evaluate_recommender(
                    recommender,
                    self.test,
                    self.metrics,
                    self.out_dir,
                    recommender_name,
                    self.features_from_test,
                ...
                print("calculating metrics...")
                evaluation_result["model_build_time"] = (
                    build_time_end - build_time_start
                )
                evaluation_result["model_inference_time"] = (
                    evaluate_time_end - evaluate_time_start
                )
                evaluation_result["model_metadata"] = copy.deepcopy(
                    recommender.get_metadata()
                )
                print("done")
