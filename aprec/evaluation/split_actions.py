...
class TemporalGlobal(ActionsSplitter):
    ...
    def __call__(self, actions):
        ...
        actions_list = sorted(
            [action for action in actions], key=lambda action: action.timestamp
        )
        cummulative = 0.0
    ...
    def __call__(self, actions):
        ...
        for action in sorted_actions:
            if self.recently_interacted_hours is not None:
                if (
                    latest_action_time - action.timestamp
                ) * 3600 < self.recently_interacted_hours:
        ...
        if self.remove_single_actions:
            ...
            valid_user_selection = list(
                eligible_users - control_users - single_action_users
            )
        else:
        ...
        test_user_ids = set(
            np.random.choice(valid_user_selection, self.max_test_users, replace=False)
        )
        for user_id in users:
            if user_id in test_user_ids:
                ...
                train += users[user_id]
        return sorted(train, key=lambda x: x.timestamp), sorted(
            test, key=lambda x: x.timestamp
        )
...
class RandomSplit(ActionsSplitter):
    ...
    def __call__(self, actions):
        ...
        test_user_ids = set(
            np.random.choice(
                list(valid_user_selection), self.max_test_users, replace=False
            )
        )
        for user_id in users:
            if user_id in test_user_ids:
                ...
                test_action_indices = set(
                    np.random.choice(
                        range(num_test_actions), num_test_actions, replace=False
                    )
                )
