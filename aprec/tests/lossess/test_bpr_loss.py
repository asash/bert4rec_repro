...
class TestBPRLoss(unittest.TestCase):
    ...
    def test_bpr_loss(self):
        ...
        val = bpr_loss(
            K.constant([[0, 0, 1, 1], [0, 0, 1, 1]]),
            K.constant([[0.1, 0.3, 1, 0], [0, 0, 1, 1]]),
        )
        self.assertAlmostEqual(float(val), 0.4495173692703247, places=4)
        poor_pred_loss = bpr_loss(
            K.constant([[0, 0, 1, 1]]), K.constant([[1, 0.5, 0, 0]])
        )
        avg_pred_loss = bpr_loss(
            K.constant([[0, 0, 1, 1]]), K.constant([[0.1, 0.3, 1, 0]])
        )
        good_pred_loss = bpr_loss(
            K.constant([[0, 0, 1, 1]]), K.constant([[0, 0, 1, 1]])
        )
    ...
    def test_bpr_loss_with_softmax(self):
        ...
        bpr_loss = BPRLoss(
            max_positives=3, batch_size=2, num_items=4, softmax_weighted=True
        )
        val = bpr_loss(
            K.constant([[0, 0, 1, 1], [0, 0, 1, 1]]),
            K.constant([[0.1, 0.3, 1, 0], [0, 0, 1, 1]]),
    ...
    def test_bpr_truncate(self):
        ...
        val = float(
            bpr_loss(tf.constant([[0, 0, 0, 1]]), tf.constant([[0.1, 0.3, 0, 0]]))
        )
