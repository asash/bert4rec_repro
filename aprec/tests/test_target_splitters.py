...
class TestItemSplitters(unittest.TestCase):
    ...
    def test_recency_sequence_sampling_exp(self):
        from aprec.recommenders.dnn_sequential_recommender.targetsplitters.recency_sequence_sampling import (
            RecencySequenceSampling,
        )
        ...
        splitter = RecencySequenceSampling(
            max_pct=0.0, recency_importance=lambda n, k: 0.5 ** (n - k)
        )
        N = 100000
    ...
    def test_recency_sequence_sampling(self):
        ...
        self.assertEquals(
            target_lens.most_common(),
            [(5, 4098), (4, 3316), (6, 1544), (3, 965), (2, 77)],
        )
        self.assertEquals(
            target_counts.most_common(5),
            [(30, 7391), (29, 6416), (28, 5638), (27, 4719), (26, 3973)],
        )

    def test_random_spliter(self):
        from aprec.recommenders.dnn_sequential_recommender.targetsplitters.random_splitter import (
            RandomSplitter,
        )
    ...
    def test_items_masking(self):
        ...
        self.assertEquals(
            label, (5, [(1, (1, 2))])
        )  # sequence len, pairs of (masked position, masked item)
        cnt = Counter()
        for i in range(100):
