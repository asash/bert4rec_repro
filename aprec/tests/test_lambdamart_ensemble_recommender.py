...
class TestLambdaMartEnsembleRecommender(unittest.TestCase):
    def test_lambdamart_ensemble_recommender(self):
        ...
        from aprec.recommenders.lambdamart_ensemble_recommender import (
            LambdaMARTEnsembleRecommender,
        )
        ...
        other_recommenders = {"svd_recommender": SvdRecommender(128)}
        ...
        train_csv = pd.read_csv(
            os.path.join(tempdir, "ensemble_train.csv.gz"),
            compression="gzip",
            delimiter=";",
        )
        val_csv = pd.read_csv(
        ...
        prediction_csv = pd.read_csv(
            os.path.join(tempdir, "ensemble_prediction_features.csv.gz"),
            compression="gzip",
            delimiter=";",
        )
        print(train_csv)
