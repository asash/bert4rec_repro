...
class TestDnnSequentialRecommender(unittest.TestCase):
    def test_mlp_recommender(self):
        ...
        from aprec.recommenders.dnn_sequential_recommender.models.mlp_sequential import (
            SequentialMLPModel,
        )
        ...
        mlp_recommender = DNNSequentialRecommender(
            model, train_epochs=10, early_stop_epochs=5, batch_size=5
        )
    ...
    def test_sasrec_model(self):
        ...
        from aprec.recommenders.dnn_sequential_recommender.models.sasrec.sasrec import (
            SASRec,
        )
    ...
    def test_sasrec_model_sampled_target(self):
        ...
        from aprec.recommenders.dnn_sequential_recommender.target_builders.sampled_matrix_target_builder import (
            SampledMatrixBuilder,
        )
        from aprec.recommenders.dnn_sequential_recommender.targetsplitters.last_item_splitter import (
            SequenceContinuation,
        )
    ...
    def test_mlp_recommender_with_lambdarank(self):
        ...
        mlp_recommender = DNNSequentialRecommender(
            model, loss, train_epochs=10, batch_size=batch_size
        )
    ...
    def test_mlp_recommender_with_xendcg(self):
        ...
        loss = XENDCGLoss()
        mlp_recommender = DNNSequentialRecommender(
            model, loss, train_epochs=10, batch_size=batch_size, debug=True
        )
        mlp_recommender.set_val_users(val_users)
        recommender = FilterSeenRecommender(mlp_recommender)
        for action in generator_limit(get_movielens20m_actions(), 10000):
