...
class TestConfigs(unittest.TestCase):
    ...
    def validate_config(self, filename):
        ...
        self.assertTrue(
            config.DATASET in DatasetsRegister().all_datasets(),
            f"Unknown dataset {config.DATASET}",
        )
        self.assertTrue(
            isinstance(config.SPLIT_STRATEGY, ActionsSplitter),
        ...
        if hasattr(config, "SAMPLED_METRICS_ON"):
            raise Exception(
                "SAMPLED_METRICS_ON field is obsolete. Please use  TARGET_ITEMS_SAMPLER"
            )
        ...
        for recommender_name in config.RECOMMENDERS:
            ...
            self.assertTrue(
                isinstance(recommender, Recommender),
                f"bad recommender type of {recommender_name}",
            )
            del recommender
            model_cnt += 1
