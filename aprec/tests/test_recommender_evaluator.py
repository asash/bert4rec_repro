...
class TestRecommenderEvaluator(unittest.TestCase):
    def test_recommender_evaluator(self):
        ...
        actions = [
            action for action in generator_limit(get_movielens20m_actions(), 100000)
        ]
        ...
        evaluator = RecommendersEvaluator(
            actions,
            recommenders,
            metrics,
            out_dir,
        ...
        self.assertEqual(
            result,
            {
                "precision@5": 0.0078125,
                "sampled_metrics": {"precision@5": 0.039062500000000014},
                "model_metadata": {
                    "top 20 items": [
                        ("318", 556),
                        ("296", 523),
