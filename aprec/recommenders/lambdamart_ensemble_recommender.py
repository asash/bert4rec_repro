...
class LambdaMARTEnsembleRecommender(Recommender):
    def __init__(
        self,
        candidates_selection_recommender: Recommender,
    ...
    def set_out_dir(self, log_dir):
        ...
        if log_dir is not None:
            self.train_users_file = gzip.open(
                os.path.join(log_dir, "ensemble_train.csv.gz"), "w"
            )
            self.val_users_file = gzip.open(
                os.path.join(log_dir, "ensemble_val.csv.gz"), "w"
            )
            self.predictions_file = gzip.open(
                os.path.join(log_dir, "ensemble_prediction_features.csv.gz"), "w"
            )
        else:
    ...
    def rebuild_model(self):
        ...
        if self.recently_interacted_hours is not None:
            for user in self.user_actions:
                if (
                    self.latest_action_time - self.user_actions[user][-1].timestamp
                ) < self.recently_interacted_hours * 3600:
        ...
        ensemble_users = set(
            np.random.choice(ensemble_users_selection, self.n_ensemble_users)
        )
        ensemble_val_users_selection = list(
            eligible_users - set(self.val_users) - ensemble_users
        )
        ensemble_val_users = set(
            np.random.choice(ensemble_val_users_selection, self.n_ensemble_val_users)
        )
        ...
        if self.predictions_file is not None:
            self.predictions_file.write(
                f"user_id;item_id;target;{';'.join(feature_names)}\n".encode("utf-8")
            )

    def get_metadata(self):
        feature_importance = list(
            zip(self.ranker.feature_name(), self.ranker.feature_importance())
        )
        result = {}
    ...
    def get_data(self, users, log_file):
        ...
        for recommender in self.other_recommenders:
            features_list += (
                f"is_present_in_{recommender}",
                f"{recommender}_idx",
                f"{recommender}_score",
            )
        ...
        if log_file is not None:
            log_file.write(
                f"user_id;item_id;target;{';'.join(features_list)}\n".encode("utf-8")
            )
    ...
    def recommend(self, user_id, limit: int, features=None):
        ...
        if self.predictions_file is not None:
            self.log_candidates(
                self.predictions_file, user_ids, items, features, scores
            )
    ...
    def build_candidates(self, user_id):
        candidates = self.candidates_selection_recommender.recommend(
            user_id, limit=self.candidates_limit
        )
        candidate_features = {}
        ...
        for recommender in self.other_recommenders:
            cnt += 1
            recs = self.other_recommenders[recommender].recommend(
                user_id, limit=self.candidates_limit
            )
