...
class DeepMFRecommender(Recommender):
    ...
    def rebuild_model(self):
        ...
        item_cnt = np.asarray(np.sum(self.item_user_matrix, axis=1)).reshape(
            self.items.size()
        )
        ...
        for step in pbar:
            ...
            users_input = np.asarray(self.user_item_matrix[users].todense()).reshape(
                1, self.num_users_per_sample, self.items.size()
            )
            items_input = np.asarray(self.item_user_matrix[items].todense()).reshape(
                1, self.num_items_per_sample, self.users.size()
            )
            target = tf.gather(users_input[0], items, axis=1)

            target = tf.reshape(
                target, (1, self.num_users_per_sample, self.num_items_per_sample)
            )
            history = self.model.fit(
                [users_input, items_input], target, epochs=1, verbose=0
            )
    ...
    def get_model(self):
        ...
        user_input, pred_user_input, user_repr, pred_user_repr = self.get_tower(
            self.items.size(), self.num_users_per_sample, self.user_hidden_layers, 1
        )

        item_input, pred_item_input, items_repr, pred_item_repr = self.get_tower(
            self.users.size(),
            self.num_items_per_sample,
            self.item_hidden_layers,
            self.items_per_pred_iter,
        )
        ...
        pred_model = Model(
            inputs=[pred_user_input, pred_item_input], outputs=pred_output
        )
    ...
    def recommend(self, user_id, limit, features=None):
        users_input = np.asarray(
            self.user_item_matrix[self.users.get_id(user_id)].todense()
        ).reshape(1, 1, self.items.size())
        ...
        while num_items_left > 0:
            ...
            item_inputs = np.asarray(
                self.item_user_matrix[start_item:end_item].todense()
            ).reshape(
                1, min(self.items_per_pred_iter, num_items_left), self.users.size()
            )
            pad_samples = (
                0
                if num_items_left > self.items_per_pred_iter
                else self.items_per_pred_iter - num_items_left
            )
            pads = [(0, 0), (0, pad_samples), (0, 0)]
            item_inputs = np.pad(item_inputs, pads)
            pred = self.pred_model.predict([users_input, item_inputs])[0][0][
                :num_items_left
            ]
