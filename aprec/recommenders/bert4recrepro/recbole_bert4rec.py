...
class RecboleBERT4RecRecommender(Recommender):
    ...
    def build_recbole_model(self, dataset_name, yaml_filename):
        ...
        self.train_data = get_dataloader(config, "train")(
            config, self.train_dataset, None, shuffle=True
        )
        self.model = get_model(config["model"])(config, self.train_data.dataset).to(
            config["device"]
        )
    ...
    def get_user_scores(self, user_id):
        ...
        interaction = {
            "item_id_list": user_sequence.to(self.model.device),
            "item_length": user_seq_len.to(self.model.device),
        }
