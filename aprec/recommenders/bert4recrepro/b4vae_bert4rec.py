...
class B4rVaeBert4Rec(Recommender):
    ...
    def rebuild_model(self):
        ...
        with B4rVaeDataset(
            self.user_actions,
            self.val_actions,
            self.user_id.straight,
            self.item_id.straight,
        ) as dataset:
            ...
            best_model = torch.load(
                os.path.join(tempdir, "models", "best_acc_model.pth")
    ...
    def get_pred_user_sequence(self, external_user_id):
        ...
        if len(actions_seq) < self.args.bert_max_len:
            actions_seq = [0] * (
                self.args.bert_max_len - len(actions_seq)
            ) + actions_seq
        ...
        for request in tqdm.tqdm(self.items_ranking_requests):
            ...
            for item_id in request.item_ids:
                if self.item_id.has_item(item_id):
                    user_result.append(
                        (item_id, float(scores[self.item_id.get_id(item_id)]))
                    )
                else:
