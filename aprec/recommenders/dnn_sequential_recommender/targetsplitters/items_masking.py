...
class ItemsMasking(TargetSplitter):
    def __init__(
        ...
        self.random = np.random.Generator(
            np.random.PCG64(np.random.SeedSequence(random_seed))
        )
    ...
    def split(self, sequence):
        ...
        if not self.force_last:
            ...
            rss_vals = np.array(
                [self.recency_importance(self.seqence_len, pos) for pos in sample_range]
            )
            rss_vals_sum = np.sum(rss_vals)
            probs = rss_vals / rss_vals_sum
            mask_positions = self.random.choice(
                sample_range, n_masks, p=probs, replace=False
            )
        else:
            n_masks = 1
