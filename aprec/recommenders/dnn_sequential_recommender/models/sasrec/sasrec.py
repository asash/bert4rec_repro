...
class OwnSasrecModel(keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.positions = tf.constant(
            tf.tile(
                tf.expand_dims(tf.range(self.max_history_length), 0),
                [self.batch_size, 1],
            )
        )

        self.item_embeddings_layer = layers.Embedding(
            self.num_items + 1, output_dim=self.embedding_size, dtype="float32"
        )
        self.postion_embedding_layer = layers.Embedding(
            self.max_history_length, self.embedding_size, dtype="float32"
        )
        ...
        for i in range(self.num_blocks):
            block_layers = {
                ...
                "attention_layers": {
                    "query_proj": layers.Dense(
                        self.embedding_size, activation="linear"
                    ),
        ...
        if not self.reuse_item_embeddings:
            self.output_item_embeddings = layers.Embedding(
                self.num_items, self.embedding_size
            )

        if self.encode_output_embeddings:
            self.output_item_embeddings_encode = layers.Dense(
                self.embedding_size, activation="gelu"
            )
    ...
    def block(self, seq, mask, i):
        ...
        x = multihead_attention(
            ...
            keys,
            self.num_heads,
            self.attention_blocks[i]["attention_layers"],
            causality=True,
        )
        x = x + queries
    ...
    def get_seq_embedding(self, input_ids, training=None):
        ...
        mask = tf.expand_dims(
            tf.cast(tf.not_equal(input_ids, self.num_items), dtype=tf.float32), -1
        )
