...
class Caser(SequentialRecsysModel):
    def __init__(
        ...
        embedding_size=64,
        max_history_len=64,
        n_vertical_filters=4,
        n_horizontal_filters=16,
        dropout_ratio=0.5,
        activation="relu",
    ...
    def get_model(self):
        ...
        model_inputs = [input]
        x = layers.Embedding(self.num_items + 1, self.embedding_size, dtype="float32")(
            input
        )
        x = layers.Reshape(
            target_shape=(self.max_history_length, self.embedding_size, 1)
        )(x)
        vertical = layers.Convolution2D(
            self.n_vertical_filters,
            kernel_size=(self.max_history_length, 1),
        ...
        for i in range(self.max_history_length):
            ...
            horizontal_convolution = layers.Convolution2D(
                self.n_horizontal_filters,
                kernel_size=(horizontal_conv_size, self.embedding_size),
                strides=(1, 1),
                activation=self.activation,
            )(x)
            pooled_convolution = layers.MaxPool2D(
                pool_size=(self.max_history_length - horizontal_conv_size + 1, 1)
        ...
        if self.requires_user_id:
            ...
            user_embedding = layers.Embedding(
                self.num_users, self.embedding_size, dtype="float32"
            )(user_id_input)
        ...
        if self.user_extra_features:
            ...
            user_features = layers.Embedding(
                self.user_feature_max_val + 1, self.embedding_size, dtype="float32"
            )(user_features_input)
            user_features = layers.MultiHeadAttention(
                self.user_features_attention_heads, key_dim=self.embedding_size
            )(user_features, user_features)
            user_features = layers.Dense(
                self.embedding_size, activation=self.activation
            )(user_features)
        ...
        output = layers.Dense(self.num_items, activation=self.output_layer_activation)(
            x
        )
