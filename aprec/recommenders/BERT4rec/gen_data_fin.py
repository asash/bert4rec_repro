...
def parse_args():
    ...
    parser.add_argument(
        "--signature", default="default", help="signature_name", type=str
    )
    parser.add_argument(
        "--pool_size", default=10, help="multiprocesses pool size.", type=int
    )
    parser.add_argument(
        "--max_seq_length", default=200, help="max sequence length", type=int
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--masked_lm_prob", default=0.15, help="Masked LM probability.", type=float
    )
    ...
    parser.add_argument(
        "--dataset_name", default="./ml-1m/", help="dataset name.", type=str
    )
...
def write_instance_to_example_file(
    instances, max_seq_length, max_predictions_per_seq, vocab, output_file
):
...
def create_training_instances(
    ...
    all_documents = get_training_documents(
        all_users_raw, force_last, max_seq_length, prop_sliding_window
    )
...
def create_instances(
    ...
    if force_last:
        ...
        instances = create_instances_no_force_last(
            all_documents,
            dupe_factor,
            instances,
            mask_prob,
...
def create_instances_no_force_last(
    all_documents,
    dupe_factor,
    instances,
    mask_prob,
    ...
    def log_result(result):
        print(
            "callback function result type: {}, size: {} ".format(
    ...
    print(
        "num of instance:{}; time:{}".format(
            len(instances), time.process_time() - start_time
        )
    )
    return instances
...
def get_training_documents(
    ...
    if force_last:
        ...
        return get_training_documents_no_force_last(
            all_users_raw, max_seq_length, prop_sliding_window
        )


def get_training_documents_no_force_last(
    all_users_raw, max_seq_length, prop_sliding_window
):
    all_documents = {}
    max_num_tokens = max_seq_length  # we need two sentence
    sliding_step = (
        (int)(prop_sliding_window * max_num_tokens)
...
def create_instances_threading(
    all_documents,
    max_seq_length,
    masked_lm_prob,
    max_predictions_per_seq,
...
def mask_last(all_documents, user, max_seq_length):
    ...
    for tokens in document:
        ...
        (
            ...
            masked_lm_positions,
            masked_lm_labels,
        ) = create_masked_lm_predictions_force_last(tokens)
...
def create_masked_lm_predictions(
    tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng, mask_prob
):
    """Creates the predictions for the masked LM objective."""
...
def gen_samples(
    ...
    write_instance_to_example_file(
        instances, max_seq_length, max_predictions_per_seq, vocab, output_filename
    )
...
def main():
    ...
    print(
        f"len_train:{len(user_train)}, len_test:{len(user_test)}, usernum:{usernum}, itemnum:{itemnum}"
    )
    ...
    user_test_data = {
        "user_"
        + str(u): ["item_" + str(item) for item in (user_train[u] + user_test[u])]
        for u in user_train
        if len(user_train[u]) > 0 and len(user_test[u]) > 0
    }
    rng = random.Random(random_seed)

    vocab = FreqVocab(user_test_data)
    user_test_data_output = {
        k: [vocab.convert_tokens_to_ids(v)] for k, v in user_test_data.items()
    }
    ...
    print(
        "vocab_size:{}, user_size:{}, item_size:{}, item_with_other_size:{}".format(
