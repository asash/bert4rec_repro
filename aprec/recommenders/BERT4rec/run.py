...
parser.add_argument(
    "--do_train", default=False, type=bool, help="Whether to run training."
)

parser.add_argument(
    "--do_eval", default=False, type=bool, help="Whether to run eval on the dev set."
)

# flags.DEFINE_integer("train_batch_size", 32, "Total batch size for training.")
parser.add_argument(
    "--batch_size", default=32, type=bool, help="Total batch size for training."
)
...
parser.add_argument(
    "--num_train_steps", default=100000, type=int, help="Number of training steps."
)

parser.add_argument(
    "--num_warmup_steps", default=10000, type=int, help="Number of warmup steps."
)
...
parser.add_argument(
    "--max_eval_steps", default=1000, type=int, help="Maximum number of eval steps."
)


parser.add_argument(
    "--use_tpu", default=False, type=bool, help="Whether to use TPU or GPU/CPU."
)
...
parser.add_argument(
    "--master", default=None, type=str, help="[Optional] TensorFlow master URL."
)
...
parser.add_argument(
    "--use_pop_random", default=True, type=bool, help="use pop random negative samples"
)
...
parser.add_argument(
    ...
    default=None,
    type=str,
    help="user history filename",
    required=True,
)
parser.add_argument(
    "--save_predictions_file", default=None, type=str, help="save predictions into file"
)
parser.add_argument(
    "--save_sampled_predictions_file",
    default=None,
    type=str,
    help="save_sampled_predictions",
)
parser.add_argument(
    "--predictions_per_user",
    default=1000,
    type=int,
    help="number of predictions to save into file",
)
parser.add_argument(
    "--training_time_limit_seconds",
    default=None,
    type=int,
    help="training will stop after N seconds",
)
parser.add_argument(
    "--sampled_instances_file",
    default=None,
    type=str,
    help="file with sampled instances",
)
...
class EvalHooks(tf.estimator.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        for idx in range(len(input_ids)):
            ...
            if (
                self.sampled_output_file is not None
                and user_id in self.sampled_instances
            ):
                self.write_sampled_predictions(
                    user_id, scores, self.sampled_output_file
                )
    ...
    def after_run(self, run_context, run_values):  # pylint: disable=unused-argument
        ...
        if self.time_limit is not None and training_time >= self.time_limit:
            tf.compat.v1.logging.info(
                f"time limit: stopping training after {training_time} seconds"
            )
            raise StopIteration()
