...
BERT4REC_DATASET_URL = \
    "https://raw.githubusercontent.com/asash" + \
    "BERT4rec_py3_tf2/master/BERT4rec/data/{}.txt"
BERT4REC_DIR = "data/bert4rec"
VALID_DATASETS = {"beauty", "ml-1m", "steam"}


def get_bert4rec_dataset(dataset: str) -> List[Action]:
    if dataset not in VALID_DATASETS:
        raise ValueError(f"unknown bert4rec dataset {dataset}")
    dataset_filename = download_file(
        BERT4REC_DATASET_URL.format(dataset), dataset + ".txt", BERT4REC_DIR
    )
    actions = []
    prev_user = None
    current_timestamp = 0
